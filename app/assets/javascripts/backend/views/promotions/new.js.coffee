class App.Views.Promotions.New extends Backbone.View
  el: '#modal'
  ...
  steps: ["type_selection", "global_info", "item_selection"]
  ...
  render: ->
    ...
    switch @current_step
      when "type_selection"
        @innerView = new App.Views.Promotions.TypeSelector(translate: @translate, model: @model)
        @.$('.modal-body').html(@innerView.render().el)
        @.$('.modal-header h3').html(@translate.views.promotion.choose_promotion_type)
      when "global_info"
        @innerView = new App.Views.Promotions.GlobalForm(translate: @translate, model: @model)
        @.$('.modal-body').html(@innerView.render().el)
        @.$('.modal-header h3').html("#{@translate.views.promotion.add_promotion} - #{@translate.views.promotion[@model.type()]}")
      when "item_selection"
  ...
  createModel: ->
    ...
    @collection.create _.clone(@model.attributes),
      success: ->
        self.$el.modal('hide')
