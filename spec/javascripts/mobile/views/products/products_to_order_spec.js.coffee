describe "ProductsToOrder View Mobile", ->
  beforeEach ->
    @productstoorderview = App.Views.Products.ProductsToOrder
    @backboneView = new Backbone.View
    @backboneRenderSpy = sinon.spy(@backboneView, "render")
    @backboneViewStub = sinon.stub(window['App.Views.Products.ProductsToOrder']).returns(@backboneView)
    ...
    @productstoorderview.collection = new Backbone.Collection([
      @backboneModel1,
      @backboneModel2,
      @backboneModel3
    ])
    @productstoorderview.render

  describe "Rendering", ->
    it "Should create a products_to_order view for each product item", ->
      expect(@backboneViewStub).toHaveBeenCalledThrice

    it "renders each ProductsToOrder view", ->
