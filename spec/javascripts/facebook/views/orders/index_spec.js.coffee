describe "Index View Facebook", ->
  beforeEach ->
    @indexview = App.Views.Orders.Index

  describe "Rendering", ->
    beforeEach ->
      @backboneView = new Backbone.View
      @backboneRenderSpy = sinon.spy(@backboneView, "render")
      @backboneViewStub = sinon.stub(window['App.Views.Orders.Index']).returns(@backboneView)
    ...
    it "Should create a Order view for each order item", ->
      expect(@backboneViewStub).toHaveBeenCalledThrice

    it "render each Order view", ->
